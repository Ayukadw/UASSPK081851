...
export const NormalisasiMatrixKeputusan = () => {
  const [criteria, setCriteria] = useState<Criteria[]>([]);
  const [alternatives, setAlternatives] = useState<Alternative[]>([]);
  const [normalizedMatrix, setNormalizedMatrix] = useState<Record<string, Record<number, number>>>({});
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resA, resC, resStep3] = await Promise.all([
          AlternativeService.getAll(),
          CriteriaService.getAll(),
          MarcosService.getStep3().catch(() => null),
        ]);
        ...
        if (resStep3 && resStep3.data && resStep3.data.success) {
          const { matrix: backendMatrix, ideal: backendIdeal, anti_ideal: backendAntiIdeal } = resStep3.data.data;
          const normMap: Record<string, Record<number, number>> = {};
          
          Object.keys(backendMatrix).forEach((altId) => {
            normMap[altId] = backendMatrix[altId];
          });
          normMap['AI'] = backendIdeal;
          normMap['AAI'] = backendAntiIdeal;
          
          setNormalizedMatrix(normMap);
        }
      } catch (error) {
        console.error('Gagal memuat data matriks normalisasi:', error);
  ...
  const dataSource = useMemo(() => {
    const rows = finalAlternatives.map((alt) => {
      ...
      finalCriteria.forEach((c) => {
        row[c.id] = normalizedMatrix[alt.id]?.[c.id] ?? 0;
      });
      return row;
    });
    ...
    finalCriteria.forEach((c) => {
      aiRow[c.id] = normalizedMatrix['AI']?.[c.id] ?? 0;
      aaiRow[c.id] = normalizedMatrix['AAI']?.[c.id] ?? 0;
    });

    rows.push(aiRow, aaiRow);
    return rows;
  }, [finalAlternatives, finalCriteria, normalizedMatrix]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <Title level={2} style={{ color: '#1D5EC9', margin: 0, fontWeight: 'bold', letterSpacing: '0.5px' }}>
        MATRIK NORMALISASI KEPUTUSAN (N)
      </Title>
